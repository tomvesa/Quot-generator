...
function printQuote(){
  // check if auto refresh is ongoing and if new quote was triggered by the button
  // if so then stop refressing clear isAuto and the checkbox
  if(isAuto && isButtonClicked){
  ...
  let quoteTextHTML = `<p class="quote">${quoteSelected.quote}</p>
      ...
      const body = document.querySelector('body');      
        ...
        isButtonClicked = false; //make sure Button controller is false
}
 ...
 * - set  interval variable => to start when clicked the checkbox
 * - button listener to check if new quote is generated by click and if 
 *      auto refresh is going on
 * - isAuto = to check if auto refresh is going on
 */
 ...
 const newQuoteBtn = document.getElementById('load-quote');
 
 // button is being clicke
...
autorefresh.addEventListener('click', e => {
  ...
  if(!isAuto){
    ...
    refreshInterval = setInterval(printQuote, 8000);
    isAuto = true;
